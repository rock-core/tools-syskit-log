...
module Syskit
    module Log
        module RoverSupport
            def self.build_aligned_vectors(
                center_time, builders, joint_stream, size, timeout: nil
            )
                ...
                joint_stream.raw_each do |index, time, sample|
                    if row_count == size
                        size, vectors = aligned_vectors_grow(size, vectors, builders)
                    end
            ...
            def self.aligned_vectors_grow(current_size, vectors, builders)
                size =
                    if current_size < 128
                        current_size * 2
                    else

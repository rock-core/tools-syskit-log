        ...
        module Daru
            ...
            class FrameBuilder
                ...
                ResolvedField = Struct.new :name, :path, :type, :transform, :vector_transform do
                    ...
                    # @api private
                    #
                    # Mapping of type properties to the corresponding Numo vector
                    #
                    # Each tuple key is [is_numeric, is_integer, is_unsigned]
                    NUMO_VECTORS = {
                        true => {
                            true => {
                    ...
                    def numo_vector_t
                        return Numo::RObject unless type <= Typelib::NumericType
                        ...
                        Rover::Vector::TYPE_CAST_MAPPING.fetch(normalized_typename.to_sym)
                    end

                    def create_vector(size)
                        numo_vector_t.new(size)
                    end
                ...
                def update_row_na(vectors, row, na_values)
                ...
                def recenter_time_vectors(vectors, center_time)
                    ...
                    @time_fields.each do |field_index|
                        ...
                        vectors[field_index] =
                            (vectors[field_index] - center_time_usec)
                            .cast_to(Numo::DFloat) / 1_000_000
                ...
                def to_daru_frame(center_time, stream, timeout: nil)
                    Daru.create_aligned_frame(
                        center_time, [self], SingleStreamAdapter.new(stream),
                        stream.size, timeout: timeout
                    )
                end
